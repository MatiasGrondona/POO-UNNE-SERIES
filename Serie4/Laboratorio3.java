...
public class Laboratorio3
{
     ...
     * @param p_diaEntrega
     */
    public Laboratorio3(String p_nombre, String p_domicilio, String p_telefono, int p_compraMin, int p_diaEnt)
    {
     ...
     * @param p_telefono
     */
    public Laboratorio3(String p_nombre, String p_domicilio, String p_telefono)
    {
        this.setNombre(p_nombre);
        this.setDomicilio(p_domicilio);
        this.setTelefono(p_telefono);
    ...
    /**
     * Retorna los dias restantes hasta la proxima entrega, contando como que no todos los meses tienen 30 dias y el siguiente dia de 
     * entrega puede ser no laborable.
    ...
    public int diasParaEntrega(){
        ...
        Calendar diaLaboral = new GregorianCalendar();
        if(diaHoy.get(Calendar.DATE) > this.getDiaEntrega()){
            diasRestantes = diaMes - diaHoy.get(Calendar.DATE) - this.getDiaEntrega();
            
            //verifica que la suiguiente fecha de entrega sea no laborable
            diaLaboral.set(anioActual,mesActual + 1,diaEntrega);
            if(diaLaboral.get(Calendar.DAY_OF_WEEK) == 7){
                diasRestantes = diasRestantes + 2;
            }else if(diaLaboral.get(Calendar.DAY_OF_WEEK) == 1){
            ...
            //verifica que la suiguiente fecha de entrega sea no laborable
            diaLaboral.set(anioActual,mesActual,diaEntrega);
            if(diaLaboral.get(Calendar.DAY_OF_WEEK) == 7){
                diasRestantes = diasRestantes + 2;
            }else if(diaLaboral.get(Calendar.DAY_OF_WEEK) == 1){
                diasRestantes = diasRestantes + 1;
